...
public class PoolingHttpClient {
	 ...
	 * @return SimpleHttpResponse Object
	 * @throws IOException IOException
	 */
	public SimpleHttpResponse doGet(String url, Map<String, String> params)
			throws IOException {
		HttpGet getMethod;
		if (params.keySet().isEmpty()){
			getMethod = new HttpGet(url);
		}else {
			StringBuilder rebuildUrl = new StringBuilder();
			rebuildUrl.append("?");
			for ( String key: params.keySet() ) {
				String param = key + "=" + params.get(key);
				rebuildUrl.append(param);
			}
			getMethod = new HttpGet(rebuildUrl.toString());
		}
	...
	public SimpleHttpResponse doGet(String url) throws IOException {
	 ...
	 * @throws IOException IOException
	 */
	public SimpleHttpResponse doDelete(String url, Map<String, String> params)
			throws IOException {
		...
		if (params.keySet().isEmpty()){
			...
			StringBuilder rebuildUrl = new StringBuilder();
			rebuildUrl.append(url);
			rebuildUrl.append("?");
			for ( String key: params.keySet() ) {
				String param = key + "=" + params.get(key) + "&";
				rebuildUrl.append(param);
			}
			deleteMethod = new HttpDelete(rebuildUrl.substring(0,rebuildUrl.length()-1));
		}

		HttpResponse response = execute(deleteMethod);
		int statusCode = response.getStatusLine().getStatusCode();
